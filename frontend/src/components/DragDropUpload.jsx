      ...
      {fileConfigs.map((cfg) => {
        ...
        return (
          <div
            ...
            className={`relative glass-panel rounded-2xl p-5 border-2 border-dashed transition-all cursor-pointer flex flex-col justify-between ${
              selectedFile
          ...
          >
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  <FileSpreadsheet className={`w-5 h-5 ${selectedFile ? "text-emerald-600" : "text-blue-600"}`} />
                  <h3 className="font-bold text-slate-800 text-sm">{cfg.title}</h3>
                </div>
                {selectedFile ? (
                  <span className="flex items-center text-xs font-bold text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-md">
              ...
              {selectedFile ? (
                <label
                  ...
                  className="block bg-white border border-emerald-200 rounded-xl p-3 shadow-xs cursor-pointer hover:border-emerald-300"
                >
                ...
                <label
                  htmlFor={`file-input-${cfg.key}`}
                  className="block text-center py-6 border border-slate-200 bg-white/60 hover:bg-white rounded-xl cursor-pointer transition-all"
                >
            ...
            <div className="mt-4 pt-3 border-t border-slate-200/60 text-[11px] text-slate-500 space-y-0.5">
              <p className="font-semibold text-slate-700">{cfg.sheetReq}</p>
